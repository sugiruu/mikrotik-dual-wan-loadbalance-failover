...
:local lISP1MonitorIP1 "208.67.222.222"
:local lISP1MonitorIP2 "9.9.9.9"
:local lISP2MonitorIP1 "208.67.220.220"
:local lISP2MonitorIP2 "149.112.112.112"
...
:do { /ip firewall mangle remove [/ip firewall mangle find where comment~"PCC" || comment~"Route" || comment~"ISP" || comment~"Return" || comment~"Output" || comment~"Bypass" || comment~"Local:" || action="mark-connection" || action="mark-routing"] } on-error={}
...
add chain=forward protocol=tcp tcp-flags=syn action=change-mss new-mss=1400 passthrough=yes comment="BS: Clamp MSS to 1400 (Safe for PPPoE/LTE)"
                    ...
                    :if (\$ISP1FailCount >= \$FailureThreshold) do={
                        ...
                        :foreach i in=[/ip route find where comment~\"ISP1\"] do={
                            :do {
                                ...
                                :if (\$c!~\"Monitor\" && \$c!~\"Failover\" && \$c!~\"FAILSAFE\") do={
                                    /ip route disable \$i
                                }
                            } on-error={}
                        }
