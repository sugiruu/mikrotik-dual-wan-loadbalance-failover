...
### Automatic Failover
- ✅ **Dual-IP Monitoring** - Monitors TWO reliable IPs (e.g., OpenDNS & Quad9) for EACH ISP
- ✅ **Conflict-Free** - Uses independent monitor IPs (208.67.222.222, 9.9.9.9) separate from your DNS (1.1.1.1, 8.8.8.8) to prevent routing loops during failover.
...
- ✅ **Comprehensive Cleanup** - Kills stuck connections with 500ms stabilization delay, then disables only PCC classification rules (preserves return traffic routing for seamless failover)
- ✅ **MSS Clamping** - TCP MSS clamped to 1400 bytes for reliable operation across PPPoE, LTE, and VPN links
...
- ✅ **Email Notifications** - Configurable alerts for 1h, 6h, Daily Reminders, and **Recovery Notifications** (only sent if outage exceeded 1 hour).
...
:local lISP1MonitorIP1 "208.67.222.222" # OpenDNS
:local lISP1MonitorIP2 "9.9.9.9"        # Quad9
:local lISP2MonitorIP1 "208.67.220.220" # OpenDNS
:local lISP2MonitorIP2 "149.112.112.112" # Quad9
...
```
Router → Ping 208.67.222.222 (OpenDNS) via ISP1 → ✅ UP
      → Ping 9.9.9.9 (Quad9) via ISP1 → ❌ DOWN
      RESULT: ISP1 is UP (At least one monitor is reachable)

Router → Ping 208.67.222.222 via ISP1 → ❌ DOWN
      → Ping 9.9.9.9 via ISP1 → ❌ DOWN
...
When ISP1 fails:
1. Scheduler detects failure (both IPs down for 2 consecutive checks = 20 seconds)
2. Script waits 500ms for connection state sync
3. Script disables ISP1 routes and PCC classification rules (return traffic rules stay enabled)
4. Clients reconnect automatically via ISP2
5. **Result**: 10-30 second interruption instead of 2-5 minute hang
...
```routeros
:local lFailureThreshold 2     # Consecutive failures before marking DOWN (default: 2 = 20 seconds)
```

The check interval is hardcoded to 10 seconds in the scheduler.
...
*   **Recovery Email**: Sent when ISP comes back online (only if outage was 1+ hours).
